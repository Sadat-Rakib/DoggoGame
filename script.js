...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateScoreDisplay() {
        indicator.textContent = `Score: ${score}`;
    }
    updateScoreDisplay();
    ...
    // Set initial dog position to center
    dog.style.left = `${window.innerWidth / 2 - 48}px`;
    dog.style.top = `${window.innerHeight / 2 - 48}px`;

    // Move dog toward bone every 20ms
    setInterval(() => {
        const dogCenter = getDogCenter();
        const boneCenter = getBoneCenter();

        const dx = boneCenter.x - dogCenter.x;
        const dy = boneCenter.y - dogCenter.y;
        const dist = Math.hypot(dx, dy);

        if (dist > 5) {
            const moveX = dx / dist * 2;
            const moveY = dy / dist * 2;
            const left = (parseFloat(dog.style.left) || window.innerWidth / 2 - 48) + moveX;
            const top = (parseFloat(dog.style.top) || window.innerHeight / 2 - 48) + moveY;
        ...
        if (checkBoneCollision()) {
            ...
            updateScoreDisplay();
            const boneRect = bone.getBoundingClientRect();
            showParticles(boneRect.left + boneRect.width / 2, boneRect.top + boneRect.height / 2);
            placeBoneRandomly();
        }
    }, 20);
});
